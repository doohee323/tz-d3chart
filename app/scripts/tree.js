    ...
    .json(
        ...
        function(raw) {
          ...
          for (var i = 0; i < rules.length; i++) {
            var tmp = rules[i];
            tmp.id = tmp.rule_id;
            if (tmp.action_meta.resources.length > 0) {
          ...
          for (var i = dataset.length - 1; i > 0; i--) {
            if (!dataset[i - 1].children) {
              dataset[i - 1].children = [];
              dataset[i - 1].children[0] = dataset[i];
              dataset[i - 1].status = status;
            } else {
              dataset[i - 1].children[dataset[i - 1].children.length] = dataset[i];
            }
          }
          for (var i = dataset.length - 1; i >= 0; i--) {
            var status = 0;
            var resources = dataset[i].action_meta.resources;
            for (var j = 0; j < resources.length; j++) {
              if (resources[j].status == 1) {
                status = 1;
                break;
              }
            }
...
function update(source) {
  ...
  nodeEnter.append("svg:text").attr("y", function(d) {
    ...
    if (d.action_meta) {
      ...
      if (d.name.indexOf('::') > -1) {
        return d.name.substring(d.name.lastIndexOf('::') + 2, d.name.length);
      } else {
        return d.name;
      }
    }
  ...
  nodeUpdate.select("circle").attr("r", 4.5).style("fill", function(d) {
    if (d._children) {
      ...
      if (d.status) {
        return "green";
      } else if (d.depth == (dataset.length + 1) || d.depth == 0) {
        return 'red';
      } else if (d.rule_id) {
        return 'red';
      } else {
  ...
  link.transition().duration(duration).attr("d", diagonal).style("stroke",
      function(d) {
...
function tooltip(txt, width) {
  ...
  var tooltip = d3.select("body").append("div").attr('pointer-events', 'none')
      .attr("class", "tooltip").style("opacity", 1).html(html).style("width",
          width + "px").style("left", (d3.event.pageX + 2 + "px")).style("top",
          (d3.event.pageY + 2 + "px"));
}
